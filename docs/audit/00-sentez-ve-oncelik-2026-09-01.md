...
- **Kademe 3–4'ten kalanlar** (Parça 1) — ~~`externalCache` kalıbı 22 noktada (D-B7)~~ → **KAPANDI (2 Eylül gecesi, 5 commit):** `common/cache/external-cache.service.ts` (`remember/readFresh/read/write`, 9 test), 16 servisin tamamı taşındı, her kanat deploy sonrası canlıda bayt bayt kıyaslandı; "fetchedAt eksik" yanlış alarmdı ama `write` artık iki dalda da yazıyor; ~~D-F2 FilmCurator↔ShowCurator~~ → **YAPILDI (2 Eylül gecesi):** gövde `components/media/TmdbCurator.tsx` + tek CSS; salon dosyaları `WING` tanımına indi (1195 → 770 satır, −425). `t` prop'la iner (`check:i18n` literal `useTranslations` ister). Canlıda küratör girişiyle sınandı: ekle/öner/ele/geri al/favori/durum/kaldır, iki salonda da çalışıyor (17:00 UTC). **Bununla denetimin bütün Critical ve High bulguları kapandı;** movies↔shows kalan ikizler (D-B4: showcase/buildLinks/buildStats — TMDB istemci soyutlaması ister); rota literalleri film/dizi/anime ayakları (H-F3'ün kalanı); öksüz oyuncu sayfası (F-4, karar gerektirir); D-F7 shelves fabrikası (Low). `music-playlist`/`music-sync` slug varyantları kendi imzalarıyla duruyor.
