...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 2007);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(2007);
  return (
    <ShikamaruExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
