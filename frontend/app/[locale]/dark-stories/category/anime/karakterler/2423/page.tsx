...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 2423);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(2423);
  return (
    <JiraiyaExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
