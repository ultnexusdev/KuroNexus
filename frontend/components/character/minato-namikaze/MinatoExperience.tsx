...
/**
 * Minato Namikaze — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`minato-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="minato-namikaze"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function MinatoExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="minato-namikaze">
      {portrait ? (
        <Image
          src={portrait}
