...
/**
 * Iruka Umino — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`iruka-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="iruka-umino"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function IrukaExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="iruka-umino">
      {portrait ? (
        <Image
          src={portrait}
