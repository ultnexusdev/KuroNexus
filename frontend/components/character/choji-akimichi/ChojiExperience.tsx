...
/**
 * Chōji Akimichi — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`choji-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="choji-akimichi"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function ChojiExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="choji-akimichi">
      {portrait ? (
        <Image
          src={portrait}
