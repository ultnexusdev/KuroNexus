...
/**
 * Kankurō — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`kankuro-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="kankuro"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function KankuroExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="kankuro">
      {portrait ? (
        <Image
          src={portrait}
