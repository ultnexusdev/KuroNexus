...
import { ShirayukiExperience } from "@/components/character/rukia-kuchiki/ShirayukiExperience";

/**
 * Rukia Kuchiki — AniList #6 (Bleach).
 *
...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 6);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(6);
  return (
    <ShirayukiExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
