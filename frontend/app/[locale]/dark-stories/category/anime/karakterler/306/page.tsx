...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 306);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(306);
  return (
    <RockLeeExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
