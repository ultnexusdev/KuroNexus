...
/**
 * Konan — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`konan-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="konan"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function KonanExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="konan">
      {portrait ? (
        <Image
          src={portrait}
