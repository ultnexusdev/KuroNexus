...
/**
 * Tsunade Senju — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`tsunade-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="tsunade"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function TsunadeExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="tsunade">
      {portrait ? (
        <Image
          src={portrait}
