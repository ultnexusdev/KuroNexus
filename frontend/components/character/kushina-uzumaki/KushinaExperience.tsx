...
/**
 * Kushina Uzumaki — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`kushina-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="kushina-uzumaki"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function KushinaExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="kushina-uzumaki">
      {portrait ? (
        <Image
          src={portrait}
