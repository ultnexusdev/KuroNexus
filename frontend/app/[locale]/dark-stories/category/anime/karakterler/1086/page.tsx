...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 1086);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(1086);
  return (
    <AizenExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
