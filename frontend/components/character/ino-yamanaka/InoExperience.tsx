...
/**
 * Ino Yamanaka — deneyim sayfası. **İSKELET SÜRÜM.**
 *
 * 24 Ağustos 2026'da rota ve deri ile birlikte açıldı; sayfanın tasarımı
 * ayrı bir dalda (`ino-redesign`) yazılıyor. İskelet bilerek ayakta: dal
 ...
 * ve kök öğede `className={styles.page}` + `data-world="ino-yamanaka"`
 * ikilisini korumalı — deri bloğu o seçiciye bağlı.
 */
export function InoExperience({ detail }: CharacterExperienceProps) {
  const portrait = primaryPortrait(detail);
  return (
    <div className={styles.page} data-world="ino-yamanaka">
      {portrait ? (
        <Image
          src={portrait}
