...
export async function generateMetadata({
  ...
  return experienceMetadata(locale, 5);
}

export default async function Page() {
  const { detail, isAdmin, companions } = await loadExperiencePage(5);
  return (
    <IchigoExperience detail={detail} isAdmin={isAdmin} companions={companions} />
  );
}
