...
- [x] **13 anime karakter deneyim sayfası (2026-08-23, dal `anime/karakter-deneyimleri`)**: Itachi (14) emsali on üç karaktere daha uygulandı — Naruto 17, Sasuke 13, Ichigo 5, Kakashi 85, Sakura 145, Urahara 210, Shikamaru 2007, Aizen 1086, Jiraiya 2423, Hinata 1555, Kenpachi 909, Rock Lee 306 ve **kap sayfası** (Sukuna 133701 + Itadori 127212 → tek sayfa, iki adres, açılış modu adrese bağlı). 99 dosya, +51 471 satır. **Ayrıntılı devir notu: `docs/DEVIR-2026-08-23-karakter-deneyim-sayfalari.md`**.
  - **Çalışma tarzı**: 13 ayrı git worktree + ayrı dal (`naruto-redesign` … `sukuna-itadori-redesign`), her biri paralel bir ajanla. Hiçbir sayfa diğeriyle bileşen paylaşmıyor (küratör modu şartı); Itachi'nin dosyalarına dokunulmadı.
  - **⚠️ AniList API'si o gün KAPALIYDI** (`403 — temporarily disabled`). Künyeler kendi backend'imizin 30 günlük önbelleğinden alındı. Canlıda sorun değil: `anilist.service.ts::getCharacter` hata durumunda bayat önbelleği sunuyor. Ama `getCharacterCards` ucunun stale-fallback'i **yok** ve önbellek anahtarı istenen kimlik kümesinin tamamından türüyor → yeni bir küme kaynak kapalıyken boş döner. Bu yüzden deneyim sayfalarının yoldaş portreleri o uçtan değil kendi `CharacterImage` kaydımızdan okunuyor.
  - **Rota yapısı değişti**: her deneyim karakteri artık kendi statik rota klasöründe (`karakterler/17/page.tsx` gibi), `[characterId]` yeniden **yalnız** künye dossier'i. Sebep ölçüldü: on dört sayfa tek rotada toplandığında App Router bütün CSS modüllerini o rotanın HTML'ine `<link>` olarak koyuyordu — sayfa başına **19 stil dosyası / 718 KB**, üstelik elle tasarlanmış sayfası olmayan 180+ karakterin künye sayfası da aynı yükü taşıyordu. `import()` ile dinamik yükleme çözmedi. Statik parça dinamik parçadan önce eşleştiği için **adresler değişmedi**; sayı 7'ye (canlı taban) indi, rota JS'i 54,2 kB → 6,1 kB.
  - **Görsel politikası**: hero portresi AniList künye portresi ya da (Naruto kadrosu + Kenpachi için) kendi yüklediğimiz tam boy portre; bütün motifler **elle çizilmiş SVG**; dış raster görsel yok (Fandom/wiki lisansı doğrulanamıyor, CSP zaten engelliyor). Sahne/dönem/teknik görselleri **üretilmedi** — **188 `ABILITY` yuvası boş**, küratör modundan yüklenecek. Görsel yokken bölümler görselsiz ama ayakta çiziliyor.
  - **Denetim**: tsc temiz · eslint 0 hata (1 önceden var olan uyarı) · `next build` başarılı · 13 CSS modülünde **tek hex yok** (kural 16) · 1519 `styles.X` referansının hepsi karşılanıyor · 360 px'te 15 sayfada da yatay taşma yok · sayfa başına tek `<h1>` · her sayfada `prefers-reduced-motion` battaniyesi ve `:focus-visible` · JSX'te sabit metin yok (hepsi `LocalizedText` + `pick()`) · TR/EN 28 adres 200.
  - Sitemap'e 14 deneyim adresi eklendi (`EXPERIENCE_IDS`, ek ağ isteği yok).

- [x] **22 karakter sayfası daha + dizin sergisi (2026-08-24)**: Sai, Yamato, Iruka, Konohamaru, Chōji, Ino, Kiba, Shino, Neji, Tenten, Gaara, Temari, Kankurō, Tsunade, Orochimaru, Kabuto, Obito, Madara, Nagato, Konan, Minato, Kushina. **Toplam elle tasarlanmış sayfa: 36 karakter / 37 adres.** 178 dosya, +95 081 satır. **Ayrıntılı devir notu: `docs/DEVIR-2026-08-24-karakter-sayfalari-2.md`**.
  - **AniList API'si 23 Ağustos'ta yeniden AÇILDI**; numaralar doğrudan GraphQL'den doğrulandı (medya listesinde Naruto/Boruto süzgeciyle). Üç sürpriz: Kankurō **4694**; **Obito → #3149, AniList'teki adı "Tobi"**; **Nagato → #3180, AniList'teki adı "Pain"** (ikisinde de ayrı kayıt yok). Yan etkisi: paylaşım kartı ve sekme adı "Tobi"/"Pain" yazıyordu → `experienceMetadata` artık kadro kaydındaki adı tercih ediyor.
  - **⚠️ MİMARİ DEĞİŞİKLİK — paletler globals.css'ten ÇIKTI.** 13 palet orada 5,4 KB gzip yer tutuyordu ve her sayfada iniyordu; 35 palet 14,5 KB olacaktı. Rotalar sayfa başına ayrıldığı için palet artık karakterin **kendi CSS modülünde**, seçici `.page[data-world=…]` (CSS Modules "pure selector" kuralı çıplak `:global([data-world])`i reddediyor). globals.css'te yalnızca `itachi`/`akatsuki`/`bleach` kaldı. Taşınan 331 token değeri tabanla birebir aynı — hem dosyadan hem çalışan sunucunun hesaplanmış değerlerinden doğrulandı. **Yeni karakter sayfası açan: paleti globals.css'e YAZMA.**
  - **Dizin sergisi**: `lib/characters/roster.ts` (37 adresin kadro kaydı — ad kodda, portre önce kendi DB'mizden sonra AniList kartından) + `CuratedShelf` (dizinin üstünde raf, sunucu bileşeni, prop olarak geçiyor). AniList kadro listeleri yalnızca başrol/yardımcıyı taşıdığı için Iruka, Konohamaru, Minato, Kushina, Tenten, Temari, Sai, Yamato, Kankurō, Kabuto sayfaları yazılsa bile dizinden ulaşılamıyordu.
  - **⚠️ AYNI GÜN REVİZE EDİLDİ (kullanıcı geri bildirimi) — sayfa İKİYE ayrıldı.** Üstte raf (kendi sayfası olan 37 adres), altta ızgara (**sayfası OLMAYAN** 50 karakter). Elle tasarlanmışlar ızgaradan tamamen düşürüldü: artık çift portre yok. "Kendi sayfası var" rozeti **kaldırıldı** — ızgarada görünen her kart zaten "sayfası yok" demek; ızgaranın üstüne bunu söyleyen bir başlık kondu (`character.rest.*`). **"Başrol / Yardımcı" çipleri de kaldırıldı** (hem dizin kartlarından hem künye sayfasının hero'sundan); rol yalnızca künye sayfasındaki "Göründüğü Yapımlar" satırlarında kaldı. Sayaç şeridi: Karakter **87** / Elle tasarlanmış **37** / Seri **9** ("Başrol" sayacı yerini aldı).
  - **Yeni karakter sayfası açarken**: `roster.ts`e satır ekle, yoksa sayfa var olur ama ne rafta görünür ne de ızgaradan düşer (yani iki yerde birden çıkar).
  - **📘 YENİ OTURUMDA İLK OKUNACAK: `docs/KARAKTER-SAYFASI-EKLEME.md`** — bir karakter daha eklemek için gereken her şey orada: dört kayıt noktası, AniList numarası çözme (ad tek başına güvenilmez), palet eşikleri ve `-text` adlandırma sözleşmesi, bileşen sözleşmesi, worktree/junction düzeni, **kullanılmış 36 mekaniğin listesi** (tekrarlanmasın diye), kullanılmış palet/önek tablosu ve bilinen tuzaklar. Geçmişin anlatısı iki DEVIR dosyasında; kılavuz "nasıl yapılır"ı anlatıyor.
  - **`npm run check:karakter`** (frontend) — dört denetim: kayıt tutarlılığı (dört yerden biri eksikse), tanımsız `styles.X` (sessiz `className={undefined}`; iki turda EN SIK bulgu buydu), deri bloğu dışında hex, palet kontrastı + accent ayrıklığı. Betikler `frontend/scripts/check-karakter-*.mjs`; bozuk durumla sınandı, yakalıyor.
  - **Renk**: 22 palet + yardımcı aile kontrast ölçülerek; ayrıca 35 accent birbirine uzaklığa göre denetlendi ve 15'ten yakın yedi çift düzeltildi. Kalan bilinen yakınlıklar (kabul edildi): Urahara~Jiraiya 10, Urahara~Kenpachi 18, Tenten~Kushina 18.
  - **Görsel**: dış raster yok; motiflerin tamamı elle çizilmiş SVG. **417 yeni `ABILITY` yuvası boş** (ilk turdaki 188 ile toplam **605**) — küratör modundan doldurulacak.
  - **Denetim**: tsc temiz · eslint 0 hata (1 önceden var olan uyarı) · build temiz · 22 modülde fazladan hex YOK · tanımsız `styles.X` 0 · 360 px'te taşma 0 · 37 adres × 200, hepsi doğru dünya ve doğru başlıkla. Denetimde **yedi hata** yakalanıp düzeltildi (ayrıntı devir notunda).
  - **⚠️ ÇALIŞMA YÖNTEMİ DERSİ**: iki dalganın ikisi de oturum limitine takıldı ve ajanlar rapor döndüremedi, ama dosyalar diskteydi. Madara'nın işi de commit edilmişti ve `git status` temiz göründüğü için önce "üretmemiş" sanıldı. **Ajan raporu gelmese bile worktree'yi `git status --short` VE `git log` ile tara.**

- [x] **5 Jujutsu Kaisen karakter sayfası (2026-08-25)**: Satoru Gojō (127691), Megumi Fushiguro (126635), Nobara Kugisaki (133700), Kento Nanami (133704), Suguru Getō (133699). **Toplam elle tasarlanmış sayfa: 41 karakter / 42 adres.** Kap sayfası (Sukuna/Itadori) JJK'nın tek sayfasıydı; bu beşi Tokyo Jujutsu Lisesi'nin çevresini kapatıyor. Ayrı devir notu YAZILMADI — `docs/KARAKTER-SAYFASI-EKLEME.md`'nin tabloları güncellendi, yeterli oldu.
  - **Numaralar AniList GraphQL'den doğrulandı** (hepsinin medya listesinde Jujutsu Kaisen var). Bu turda **ad ayrışması YOK**: AniList "Satoru Gojou" ve "Suguru Getou" yazımını kullanıyor, kadro kaydı da aynısını taşıyor — yani Obito/Nagato emsali burada geçerli değil.
  - **⚠️ JJK KADROSUNUN HİÇ GÖRSELİ YOK.** `/anime/characters/images?ids=…` ölçüldü, **boş dizi** döndü. Beş sayfa da buna göre tasarlandı: hero portresi AniList'in ~230 px görseli (`unoptimized`), geri kalan her motif **elle çizilmiş SVG**, ve beş interaktif bölümün hiçbiri yüklenmiş görsele bağlı değil. Küratör yüklediğinde bölümler kendiliğinden zenginleşir.
  - **Beş yeni mekanik** (hiçbiri önceki 36'yla yapısal olarak aynı değil): Gojō — iki boş yuva, sonucu miktar değil ZITLIK belirliyor (蒼+赫→茈); Megumi — ortak zemin çizgisi, seçim üç ayrı cevap veriyor ve biri **kategorik red** (kırılmış gölge bir daha doğrulmuyor); Nobara — iki pano, solda vuruyorsun sağda oluyor, arada **bağ yoksa hiçbir şey olmuyor**; Nanami — tahmin → ölç → kes, hedef değişse de nokta hep %70'te; Getō — **tek yönlü hazne**, al/biriktir/bir kere boşalt, geri alma yok.
  - **Mod düğmeleri de birbirinden ayrı iş yapıyor**: Gojō bilgi AÇIYOR (gözbağı çözülünce gizli ölçüm satırları görünür olur), Megumi ışık DÜŞÜRÜR (`--meg-drop`), Nobara sayfanın SESİNİ yükseltir (`--nob-assert` display ölçülerini büyütür), Nanami sayfanın ORANINI değiştirir (`--nan-major` 70% → 100%, bütün çizgiler kırılmayı bırakır), Getō sayfadan RENGİ çeker (`--get-chrome`). **Hiçbiri metin token'ına dokunmuyor** — ölçülmüş kontrastlar modla bozulmuyor.
  - **Palet**: kılavuzun boş bıraktığı ailelerden üçü kapandı — bakır/bronz (nobara `#cf7a52`), soğuk gri-mavi (megumi `#8fa6bf`), mat pirinç (nanami `#b39a5c`); gojo buz mavisi `#5fd0f0`, getō kirli yosun `#83ab7d`. 42 palet ölçüldü, yeni yakın accent çifti çıkmadı.
  - **Denetim**: tsc temiz · eslint 0 hata (1 önceden var olan uyarı) · `next build` temiz · `npm run check:karakter` dört denetimde de temiz (42 adres / 42 kadro satırı, 41 modül) · 5 sayfada **sayfa başına 7 stil dosyası** (tek rota emsaliyle aynı, sızma yok) · 360 px'te beşinde de yatay taşma 0 · sayfa başına tek `<h1>` · TR ve EN adresler 200 ve içerik dile göre değişiyor.
  - **Üretim derlemesinde ELLE denendi** (port 3100, tarayıcıdan): beş mod düğmesi ve beş interaktif bölümün hepsi çalışıyor — Gojō'da 蒼+蒼 çarpıştırmayı kilitliyor / 蒼+赫 茈'yi doğuruyor, Megumi'de kırık gölge doğrulmuyor ve ritüel Mahoraga'yı açıyor (sayaç 8→9/10), Nobara'da bağ kurulmadan çakılan çivi karşı tarafta iz bırakmıyor ve bağ sonradan kurulunca **önceki çiviler geriye dönük** beliriyor, Nanami'de hedef değişse de gerçek nokta %70'te kalıyor, Getō'da girdap hazneyi sıfırlıyor ve yutulanlar geri gelmiyor.
  - **⚠️ BULUNAN VE DÜZELTİLEN**: Nobara'nın tezgâhında `setPhase` bir `setNails` güncelleyicisinin İÇİNDEN çağrılıyordu (güncelleyicinin saf kalması kuralını bozuyor, geliştirme kipinde iki kez yazıyordu). Olay işleyicisine taşındı.
  - **⚠️ HERE-DOC TUZAĞI TEKRAR ÇIKTI**: kılavuzun §4 palet tablosunu yenileyen betik `node - <<'EOF'` ile çalıştırıldığında regex sessizce boş döndü ve tabloyu **tek satıra indirdi**. Betik dosyaya yazılıp `node dosya.mjs` ile koşturulunca 41 satır doğru üretildi. **Regex içeren betiği here-doc'la besleme.**
